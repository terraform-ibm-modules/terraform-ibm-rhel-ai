locals {
    ...
    dst_rhelai_inventory_file  = "${local.dst_files_dir}/rhelai-inventory"
    ...
    ansible_inventory_file     = "${local.src_ansible_templates_dir}/rhelai-inventory"
    ...
    model_dest_dir             = "/root/.cache/instructlab/models/${var.bucket_name}"
...
resource "terraform_data" "setup_ansible_host" {
    ...
    provisioner "file" {
        content                 = templatefile(local.ansible_ilab_service_file, {
            model_name          = length(var.bucket_name) > 0 ? var.bucket_name : var.model_repo
        })
